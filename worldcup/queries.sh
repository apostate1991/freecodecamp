...
echo "$($PSQL "SELECT distinct teams.name FROM teams JOIN games ON teams.team_id = games.winner_id WHERE games.year = 2018 AND games.round = 'Final'")"

echo -e "\nList of teams who played in the 2014 'Eighth-Final' round:"
echo "$($PSQL "SELECT distinct teams.name FROM teams JOIN games ON teams.team_id = games.winner_id OR teams.team_id = opponent_id WHERE games.year = 2014 AND games.round = 'Eighth-Final' ORDER BY teams.name")"
...
echo "$($PSQL "SELECT distinct teams.name FROM teams WHERE teams.name ILIKE 'Co%'")"
